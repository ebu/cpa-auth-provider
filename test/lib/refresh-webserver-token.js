 ...
 *   example-service.bbc.co.uk (201)
 * - another web server (104) that has access token for domain
 *   example-service.bbc.co.uk (201)
 */
...
var initDatabase = function(done) {
  db.Client
    ...
    .then(function() {
      return db.Client.create({
        id:                104,
        secret:            'b6a80c14-8d5d-4ff9-bd26-c2ee602d4e3e',
        name:              'Another Webserver test client',
        software_id:       'CPA AP Test',
        software_version:  '0.0.1',
        ip:                '127.0.0.1',
        registration_type: 'static',
        redirect_uri:      'https://example-client2.com/cpa/callback'
      });
    })
    .then(function() {
    ...
    .then(function() {
      // Access token for the other web server client
      var date = new Date("Wed Apr 09 2014 11:00:00 GMT+0100");

      return db.AccessToken.create({
        token:         '2c65e100-b8bb-4fd7-8d9c-b3b44dd7e9ef',
        client_id:     104,
        user_id:       302,
        domain_id:     201,
        refresh_token: 'f7ea456f-7a94-467d-9d6d-c21350f4cb2c',
        created_at:    date,
        updated_at:    date
      });
    })
...
describe("POST /token", function() {
  ...
  context("with refresh_token grant type", function() {
    context("and a valid refresh token", function() {
      ...
      describe("the response body", function() {
        ...
        it("should include a new access token", function() {
        ...
        it("should include anew refresh token", function() {
      ...
      describe("the database", function() {
        before(function(done) {
          ...
          db.AccessToken.findAll({ where: { client_id: 103 } })
        ...
        it("should replace the existing access token", function() {
        ...
        describe("the access token", function() {
          ...
          it("should be associated with the correct client", function() {
            expect(this.accessTokens[0].client_id).to.equal(103);
          });
          ...
          it("should have a new refresh token", function() {
            expect(this.accessTokens[0].refresh_token).to.equal('6483922db068454a891253a53f5b175a');
          });
        });
      });
    });

    context("and a refresh token issued by a different client", function() {
      before(resetDatabase);
      ...
      before(function(done) {
        var requestBody = {
          ...
          client_id:     '103',
          client_secret: '399a8f42-365e-42db-a745-cf8b0e5b7ab7',
          refresh_token: 'f7ea456f-7a94-467d-9d6d-c21350f4cb2c' // belongs to client 104
          //domain:        'example-service.bbc.co.uk'
        };
    ...
    context("and a device client", function() {
      before(resetDatabase);
  ...
  var fields = ['client_id', 'client_secret', 'refresh_token'];
