...
permissionHelper.isAdmin = function (user) {
    return permissionHelper.hasPermission(user, permisssion.ADMIN_PERMISSION);
};

permissionHelper.hasPermission = function (user, permission) {
    return db.Role.findOne({where: {id: user.role_id}}).then(function (role) {
        return role && role.label === permission;
    });
};

permissionHelper.grantPermission = function (user, persmission) {
    return db.Role.findOne({where: {label: persmission}}).then(function (role) {
