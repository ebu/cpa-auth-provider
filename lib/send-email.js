...
module.exports = {
    ...
    sendDeleteCanceledNotification: sendDeleteCanceledNotification,
...
var TYPES = {
    ...
    DELETE_CANCELED_NOTE: {filename: 'delete-canceled-note.html'},
                ...
                ses.sendEmail(
                    ...
                    function (err, data) {
                        if (err) {
                            ...
                            monitor.counter.inc(monitor.METRICS.EMAIL_SENT, 1);
                ...
                ses.sendEmail(
                    {
                        ...
                        Message: {
                            Subject: {
                                Data: 'BR Mediathek Nutzerkonto löschen',
                                Charset: 'utf8'
                            },
                            Body: {
                                Html: {
...
function sendDeleteCanceledNotification(recipient, undoLink, client) {
    return new Promise(function (resolve, reject) {
        getEmail(TYPES.DELETE_CANCELED_NOTE, client).then(
            function (content) {
                console.log('sendDeleteCanceledNotification', recipient, ',', undoLink);
                var emailContent = content.replace(/\{\{USEREMAIL}}/g, recipient)
                    .replace(/\{\{LINK}}/g, undoLink);
                ...
                ses.sendEmail(
                    {
                        ...
                        Message: {
                            Subject: {
                                Data: 'BR Mediathek Nutzerkonto löschen aufgehoben',
                                Charset: 'utf8'
                            },
                            Body: {
                                Html: {
                    ...
                    function (err, data) {
                        if (err) {
                            logger.debug('[Email Sending][Delete Canceled Notification][FAIL][TO', recipient, '][FROM', FROM_ADDRESS, ']', err);
                            return reject(err);
                        } else {
                            monitor.counter.inc(monitor.METRICS.EMAIL_SENT, 1);
                            logger.debug('[Email Sending][Delete Canceled Notification][SUCCESS][TO', recipient, '][FROM', FROM_ADDRESS, ']');
                            return resolve(data);
                        }
                    }
                );
