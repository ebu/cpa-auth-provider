        ...
        if (!socialLogin) {
            ...
            db.LocalLogin.findOne({where: {login: profile.email}, include: [db.User]}).then(function (localLogin) {
                ...
                return db.sequelize.transaction(function (transaction) {
                    ...
                    return done(null, buildResponse(user, profile.email, {clientId:clientId}));
                });
            }).catch(function (err) {
            ...
            socialLogin.updateAttributes(
                ...
                return done(null, buildResponse(socialLogin.User, profile.email, {clientId:clientId}));
            }).catch(
                done
            );
        }
...
function buildResponse(user, email, extra) {
    var token = jwtHelper.generate(user.id, 10 * 60 * 60, extra);
    return {
        success: true,
        user: {
