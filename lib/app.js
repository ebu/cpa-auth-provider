...
//Fail identity provider if more than one provider is enable for now
// WAITING FOR PEACH-966
var identityProviderNb = 0;
for (var provider in config.identity_providers) {
    if (config.identity_providers[provider].enabled && config.identity_providers[provider] !== config.identity_providers.local) {
        identityProviderNb++;
    }
    if (identityProviderNb > 1) {
        throw ("Only one provider should be enabled to run the idp. Please disable some providers and try again.");
    }
}
