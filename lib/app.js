...
app.all('/admin*', function (req, res, next) {
    if (! req.isAuthenticated()){
        ...
        // redirect or 302 depending on the method type : GET : redirect, POST : 302
        if (req.method === 'GET'){
            requestHelper.redirect(res, '/auth');
        } else {
            res.sendStatus(401);
        }
    } else if(typeof req.user.Permission !== "undefined"
        && req.user.Permission !== null
        && req.user.Permission.label === permissionName.ADMIN_PERMISSION) {
        next();
    } else {
        req.session.auth_origin = req.originalUrl;
            res.sendStatus(403);
