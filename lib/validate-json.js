"use strict";

var requestHelper = require('./request-helper');

var jsonSchema = require('jsonschema');

/**
 * Returns an express middleware function that validates the request body
 * or the request's field if provided against the given JSON schema.
 *
 * - schemas is either an array of object or a single object
 * - field is the field of the req object to validate (default: body)
 * - useRedirectError defines if the error will be send in the reply or using
 * query parameters while redirecting to the redirect_uri of the req object.
 */

var validateJson = function(schemas, field, useRedirectError) {
  return function(req, res, next) {
    if (!field) {
      field = 'body';
    }

    // TODO: Use redirect_uri provided by the client.

    if (field === 'body' && !requestHelper.isContentType(req, 'application/json')) {
      res.sendInvalidRequest("Invalid content type: " + req.get('Content-Type'),
        (useRedirectError? req.query.redirect_uri : null));
      return;
    }

    if (!Array.isArray(schemas)) {
      schemas = [schemas];
    }

    var errors = [];
    schemas.forEach(function (schema, index) {
      var result = jsonSchema.validate(req[field], schema);

      if (result.errors.length > 0) {
        errors.push('Errors in schema ' + index + ': ' + result.toString());
      }
    });

    if (errors.length === schemas.length && errors.length > 0) {
      var errorMessage = '';

      errors.forEach(function(err) {
        errorMessage += err;
      });
      
      return res.sendInvalidRequest(errorMessage,
        (useRedirectError? req.query.redirect_uri : null));
    }
    next();
  };
};

module.exports = validateJson;
