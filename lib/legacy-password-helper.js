...
function createBTQHash(password, salt) {
    var passwordByte    = new Buffer(password);
    var saltByte        = new Buffer(salt);
    var passwordPlusSaltByte = bufferConcat([passwordByte, saltByte]);

    var hash = crypto.createHash("md5");
    hash.update(passwordPlusSaltByte);
    var md5WithoutSaltByte  = new Buffer(hash.digest("md5"), "md5");
    var md5WithSaltByte = bufferConcat([md5WithoutSaltByte, saltByte]);
    return BTQ + md5WithSaltByte.toString('base64');
}

function checkLegacyPassBTQ(passwordToCheck, hash, btqHash) {
    var hash = btqHash.substring(BTQ.length);
    var decodedHash = Buffer.from(hash, 'base64');
    var strDecodedHash = decodedHash.toString();
    var strSalt =  strDecodedHash.substr(strDecodedHash.length - 2);
    return createBTQHash(passwordToCheck, strSalt) === btqHash;
}
