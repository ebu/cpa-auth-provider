...
function checkLegacyPassECE(passwordToCheck, eceHash) {
    // var b64string = eceHash.substring(ECE.length);


    var buf = Buffer.from(eceHash, 'base64');
...
function checkLegacyPassSSHA(passwordToCheck, sshaHash) {
...
function registerAlgorithm(type, func) {
    algorithms[type] = function (password, hash) {
        return new Promise(function (resolve, reject) {
            return resolve(func(password, hash));
        });
    };
}
