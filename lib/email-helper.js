...
var TEMPLATE_DEFAULT_PATH = 'default';
 ...
 *                                   has completed
 */
function send(from, to, subject, templateName, opts, data, local, done) {
	if (opts.log) {
	...
	// var dirChoices = ['br-mediathek', undefined, 'pasta', 'alpha', 'jones', 'laboutique'];
	// opts.templateClass = dirChoices[Math.floor(Math.random() * dirChoices.length)];


	var templateDir = path.join(__dirname, '..', 'templates', TEMPLATE_DEFAULT_PATH, templateName);

	new Promise(
		function (resolve, reject) {
			if (!opts.templateClass) {
				return resolve(templateDir);
			}
			var checkDir = path.join(__dirname, '..', 'templates', opts.templateClass, templateName);
			fs.stat(
				checkDir,
				function (err, stats) {
					if (err) {
...
function renderEmail(name, data, locale, templateClass) {
	return new Promise(
		function (_resolve, _reject) {
			// var dirChoices = ['br-mediathek', undefined, 'pasta', 'alpha', 'jones', 'laboutique'];
			// templateClass = dirChoices[Math.floor(Math.random() * dirChoices.length)];
