 ...
 */
function createUser(email, password, requiredAttributes, optionnalAttributes) {
    return new Promise(
...
function buildSearchOptions(req) {
    ...
    if (req.query.email) {
        user_where.email = {$like: '%' + req.query.email.toLowerCase() + '%'};
    }
    ...
    if (!req.query.firstname && !req.query.lastname) {
        ...
        if (req.query.firstname) {
            profile_where.firstname = {like: '%' + req.query.firstname.toLowerCase() + '%'};
        }
        if (req.query.lastname) {
            profile_where.lastname = {$like: '%' + req.query.lastname.toLowerCase() + '%'};
        }
        options = {
            offset: offset,
            where: user_where,
...
function buildSearchQuery(req) {
    var fields = "\"Users\".id, \"Users\".tracking_uid, \"Users\".email, \"Users\".password, \"Users\".enable_sso, \"Users\".display_name, \"Users\".photo_url, \"Users\".verified, \"Users\".password_changed_at, \"Users\".last_login_at, \"Users\".created_at, \"Users\".updated_at, \"Users\".identity_provider_id, \"Users\".permission_id";
    return buildCreterions(req, fields, true);
}

function buildCountQuery(req) {
    var fields = "COUNT(*) AS \"count\"";
    return buildCreterions(req, fields, false);
}

function buildCreterions(req, fields, withLimitAndOffset) {

    // If an id is provided any other parameter are not taken into account.
    if (req.query.id) {
        return "SELECT " + fields + " FROM \"Users\" where \"Users\".id=" + req.query.id;
    }

    var select;

    if (!req.query.firstname && !req.query.lastname) {
        select = "SELECT " + fields + " FROM \"Users\" ";
        if (req.query.email) {
            select += " WHERE LOWER(Email) like '%" + req.query.email.toLowerCase() + "%'";
        }

    } else {
        select = "SELECT " + fields + " FROM \"Users\", \"UserProfiles\" WHERE \"Users\".id = \"UserProfiles\".user_id ";
        var first = false;
        if (req.query.email) {
            if (first) {
                select += " WHERE LOWER(Email) like '%" + req.query.email.toLowerCase() + "%'";
            } else {
                select += " AND LOWER(Email) like '%" + req.query.email.toLowerCase() + "%'";
            }
            first = false;
        }
        if (req.query.firstname) {
            if (first) {
                select += " WHERE LOWER(firstname) like '%" + req.query.firstname.toLowerCase() + "%'";
            } else {
                select += " AND LOWER(firstname) like '%" + req.query.firstname.toLowerCase() + "%'";
            }
        }
        if (req.query.email) {
            if (first) {
                select += " WHERE LOWER(lastname) like '%" + req.query.lastname.toLowerCase() + "%'";
            } else {
                select += " AND LOWER(lastname) like '%" + req.query.lastname.toLowerCase() + "%'";
            }
        }
    }
    ...
    if (withLimitAndOffset) {
        if (req.query.limit) {
            if (req.query.limit < MAX_LIMIT) {
                select += " LIMIT " + req.query.limit;
            } else {
                select += " LIMIT " + MAX_LIMIT;
            }
        } else {
        ...
        if (req.query.offset) {
            select += " OFFSET " + req.query.offset;
        }
    }

    logger.debug("Executing raw Query ", select);

    return select;
}

function getUsers(req) {
    return db.sequelize.query(buildSearchQuery(req), {model: db.Users});
    //return getUsersCaseSensitive(req);
}

function countUsers(req) {
    return db.sequelize.query(buildCountQuery(req), {model: db.Users});
    //return countUsersCaseSensitive(req);
}

// This function is not used since I didn't find a way to use LOWER + LIKE on firstname, lastname and email column
function getUsersCaseSensitive(req) {

    var options = buildSearchOptions(req);
...
// This function is not used since I didn't find a way to use LOWER + LIKE on firstname, lastname and email column
function countUsersCaseSensitive(req) {
