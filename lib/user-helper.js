...
var EXCEPTIONS = {
    ...
    MALFORMED_BIRTHDAY: 'MALFORMED_BIRTHDAY'
};
...
var requiredFields = {"gender": false, "birthday": false};
...
function validateFields(attributes) {
    if (attributes.hasOwnProperty('gender')) {
        if (!attributes.gender.match(/(male|female)/)) {  // attributes.gender !== 'male' || attributes.gender !== 'female') {
            throw new Error(EXCEPTIONS.UNKNOWN_GENDER);
        }
    }

    if (attributes.hasOwnProperty('birthday')) {
        if (!attributes.birthday.match(/\d\d\/\d\d\/\d\d\d\d/)) {
            throw new Error(EXCEPTIONS.MALFORMED_BIRTHDAY);
        }
    }
    return true;
}
            ...
            db.User.findOne({where: {email: email}}).then(
                ...
                function (permission) {
                    ...
                    if (missingFields.length > 0) {
                        throw new Error(EXCEPTIONS.MISSING_FIELDS, missingFields);
                    }
