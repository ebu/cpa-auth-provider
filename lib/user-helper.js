 ...
 * @param requiredAttributes user attributes (i. e. language, gender, name)
 */
        ...
        function (resolve, reject) {
            ...
            db.LocalLogin.findOne({where: {login: email}}).then(
                function (localLogin) {
                    ...
                    return db.SocialLogin.findOne({where: {$and: [{email: email}, {email: {$ne: null}}, {email: {$ne: ''}}]}});
                }).then(
                ...
                function (permission) {
                    ...
                    return db.sequelize.transaction(function (transaction) {
                        ...
                        return db.User.create(userAttributes, {transaction: transaction}).then(
                            function (u) {
                                user = u;
                                return db.LocalLogin.create({user_id: u.id, login: email}, {transaction: transaction});
                            }
                        ).then(
                            function (ll) {
                                localLogin = ll;
