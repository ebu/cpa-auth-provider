...
var requiredFields = {"gender": false, "dateOfBirth": false, "firstname": false, "lastname": false, "language": false};
...
function prepareConfig() {
    ...
    for (var i = 0; i < config.userProfiles.requiredFields.length; ++i) {
        config.userProfiles.requiredFields[i] = config.userProfiles.requiredFields[i].trim();
...
function validateFields(attributes) {
    ...
    if (attributes.hasOwnProperty('dateOfBirth')) {
        if (typeof(attributes.dateOfBirth) === 'string' && !attributes.dateOfBirth.match(/\d*/)) {
            throw new Error(EXCEPTIONS.MALFORMED_DATE_OF_BIRTH);
        } else if (typeof(attributes.dateOfBirth) !== 'string' && typeof(attributes.dateOfBirth) !== 'number') {
            throw new Error(EXCEPTIONS.MALFORMED_DATE_OF_BIRTH);
        }
    }
    ...
    return new Promise(
        function (resolve, reject) {
            ...
            db.UserProfile.findOrCreate({where: {user_id: user.id}}).spread(
                function (profile) {
                    userProfile = profile;
                    var fields = ['firstname', 'lastname', 'gender', 'dateOfBirth', 'language'];
                    var attributes = {};
